...
$ vault write auth/kubernetes/role/role-system-a-dev \
    ...
    bound_service_account_namespaces=test,test-vault \
...
Another example link:examples/vault-agent-systemb.yaml[] contains the deployment and configuration to access the `dev/system-b` secrets.
...
----
$ oc new-project test-vault
$ oc label namespace test-vault webhookinjection=enabled
namespace/test-vault labeled
...
$ helm3 upgrade --install banzai-vault-webhook . \
 -f scripts/webhook.vaules.openshift.yaml

----

In case the runtime is OpenShift does the applications needs some security adjustments for the service account `banzai-vault-webhook-vault-secrets-webhook`
...
Apply the link:examples/vault-webhook-base.yaml[] which deploys the same base application with the relevant annotations.
...
----
$ oc logs -f showcase-vault-deployment-854d8c6d48-8wq7h
...
time="2021-10-12T19:01:04Z" level=info msg="spawning process: [sh -c /scripts/add.sh]" app=vault-env
add.sh script
[Main container] ...waiting...
Env:
DB_USERID=SystemA.DB-User.Dev
